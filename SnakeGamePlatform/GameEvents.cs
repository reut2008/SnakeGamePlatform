...
using SnakeGamePlatform.Properties;
...
namespace SnakeGamePlatform
{
    ...
    public class GameEvents:IGameEvents
    {
        ...
        TextLabel lblScore, score, lblLose;
        GameObject right, left, down, up, food, backImg;
        ...
        //הגדרת משתנים וגיים אובג'קטים חשובים
        public void GameInit(Board board)
        {
            ...
            //board.SetBackgroundImage(Properties.Resources.backtmuna);
            ...
            snake[0].SetImage(Properties.Resources.betterhead);
            ...
            Position imgPos = new Position(200, 20);
            backImg = new GameObject(imgPos, 760, 780);
            backImg.SetImage(Properties.Resources.backtmuna);
            board.AddGameObject(backImg);
            ...
            misgeret[0].SetBackgroundColor(Color.DarkBlue);
        ...
        //מוסיפה אוכל במקום רנדומלי ומוחקת את האוכל הקודם
        public void AddFood(Board board)
        {
            ...
            food = new GameObject(snakePosition, 30, 30);
            food.SetImage(Properties.Resources.burger);
            board.AddGameObject(food);
          


        }
         //בונה את המערך של הנחש ככה שהוא גדל 
        public void AddSnake(Board board, GameObject[] snake)
        {
            ...
            snake[snakeSize].SetImage(Properties.Resources.snakebody);
        ...
        //בודקת אם הנחש נגע במסך או בעצמו בשביל לדעת אם לפסול
        public void Lose(GameObject[] snake, GameObject[] misgeret, Board Board, int snakeSize, Board board)
        {
            ...
            //הוספת מוזיקה ואפשרות להתחיל מחדש
            if (gameEnded)
            {
                board.PlayBackgroundMusic(@"\Images\");
                board.PlayBackgroundMusic(@"\Images\lose.wav");
                Position labelPosition = new Position(100, 400);
                lblLose = new TextLabel("לחץ רווח כדי להתחיל מחדש", labelPosition);
                lblLose.SetFont("Ariel", 14);
                board.AddLabel(lblLose);
            }
        }
        //מוסיפה נקודות
        public void Points()
        ...
        public void Reset(Board board)
        {
            ...
            board.RemoveLabel(lblLose);
            board.RemoveLabel(score);

            board.RemoveGameObject(food);
            board.RemoveGameObject(backImg);
            points = 0;
            snakeSpeed = 181;
            board.PlayBackgroundMusic(@"\Images\");
            board.PlayBackgroundMusic(@"\Images\backgroundmusic.mp4");
